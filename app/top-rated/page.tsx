...
const TopRatedPage: React.FC = () => {
  ...
  const loadMovies = async (page: number = 1, append: boolean = false) => {
    try {
      ...
      // Check if we have more pages (TMDb typically has 500+ pages for top rated movies)
      setHasMore(topRatedMovies.length === 20 && page < 500);
    } catch (err) {
      setError('Failed to load top rated movies. Please check your API key and try again.');
  ...
  return (
    <PageContainer>
      <HeroSection>
        <HeroTitle>Top Rated Movies</HeroTitle>
        <HeroSubtitle>
